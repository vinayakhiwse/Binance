...
function CurrencyModel({
  ...
  const handleSelected = (object) => {
    setTokenSelected(object);
    setTokenModel(false);
  };

  useEffect(() => {
    if (search) {
      const filterData = tokenData?.filter(
        (el) =>
            ...
            {filterdata?.map((el, i) => (
              <div
                ...
                onClick={() =>
                  handleSelected({
                    token: el.symbol,
                    price: el.quote.USD.price.toFixed(2),
                    img: "https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcQiTVS8KzlChtoGh4qLpg-7NKsTdOqjrf_CgwWUiOeN5E6sRx8u",
                  })
                }
                className="mx-5 my-2 flex justify-between cursor-pointer items-center p-3 hover:border rounded-md border border-1 border-gray-50 hover:border-gray-200"
              >
                ...
                <p className="text-md font-thin text-gray-400">{el.name}</p>
              </div>
            ))}
          </Scrollbars>
        </div>
